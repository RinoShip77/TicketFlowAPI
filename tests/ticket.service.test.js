...
describe('Ticket Service', () => {
  ...
  test('Must construct a Regex search filter if the search parameter is present', async () => {
    ...
    // On vérifie la nouvelle syntaxe Mongoose native générée par le service
    expect(filtres.title).toBeDefined();
    expect(filtres.title.$regex).toBe('Panne');
    expect(filtres.title.$options).toBe('i');
  });
